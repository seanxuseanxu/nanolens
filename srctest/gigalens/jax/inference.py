...
class ModellingSequence(gigalens.inference.ModellingSequenceInterface):
    def MAP(
        ...
        num_nodes = jax.device_count() // jax.local_device_count()
        n_samples = n_samples // num_nodes
        print(f"{n_samples} samples per node")
        ...
        seed = jax.random.PRNGKey(jax.process_index())
    ...
    def SVI(
        ...
        dev_cnt = jax.local_device_count()
        seeds = jax.random.split(jax.random.PRNGKey(jax.process_index()), dev_cnt)
